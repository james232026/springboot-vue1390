<template>
  <div class="addEdit-block">
    ...
    >
      <el-row >
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'" label="销售账号" prop="xiaoshouzhanghao">
          <el-select :disabled="ro.xiaoshouzhanghao" @change="xiaoshouzhanghaoChange" v-model="ruleForm.xiaoshouzhanghao" placeholder="请选择销售账号">
            <el-option
                v-for="(item,index) in xiaoshouzhanghaoOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input"v-if="ruleForm.xiaoshouzhanghao" label="销售账号" prop="xiaoshouzhanghao">
...
export default {
  ...
  methods: {
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error(data.msg);
            ...
            this.$http({
              url: `option/xiaoshouyuan/xiaoshouzhanghao`,
              method: "get"
            }).then(({ data }) => {
              if (data && data.code === 0) {
                this.xiaoshouzhanghaoOptions = data.data;
              } else {
                this.$message.error(data.msg);
              }
            });
    ...
    // 下二随
    xiaoshouzhanghaoChange () {
      this.$http({
        url: `follow/xiaoshouyuan/xiaoshouzhanghao?columnValue=`+ this.ruleForm.xiaoshouzhanghao,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
              if(data.data.xiaoshouxingming){
                this.ruleForm.xiaoshouxingming = data.data.xiaoshouxingming
              }
        } else {
          this.$message.error(data.msg);
        }
